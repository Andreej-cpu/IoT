...
Wokwi simulates a WiFi network with full internet access. You can use the [ESP32](esp32.html "esp32.md") together with the virtual WiFi to prototype IoT projects. Common use cases include:

* Connect to MQTT servers to send sensor data
* Query web services over HTTP, HTTPS, and web sockets
* Run an HTTP server inside the ESP32 and connect to it from your browser (requires the [Wokwi IoT Gateway](esp32-wifi.html#the-private-gateway "esp32-wifi.md#the-private-gateway"))
...
To connect from a [MicroPython project](https://wokwi.com/projects/new/micropython-esp32 "https://wokwi.com/projects/new/micropython-esp32"), use the following code:
...
Once connected, you can use the [urequests library](https://mpython.readthedocs.io/en/master/library/mPython/urequests.html "https://mpython.readthedocs.io/en/master/library/mPython/urequests.md") to send HTTP and HTTPS requests, and the [umqtt library](https://mpython.readthedocs.io/en/master/library/mPython/umqtt.simple.html "https://mpython.readthedocs.io/en/master/library/mPython/umqtt.simple.md") to establish MQTT connections.
...
Note: We need to specify the auth\_method to `None` in the ClientConfiguration.

## Custom WiFi Access Points

With a [Wokwi Hobby+ or Pro plan](https://wokwi.com/pricing?ref=docs_custom_wifi "https://wokwi.com/pricing?ref=docs_custom_wifi"), you can add custom WiFi access points to your diagram. This lets you:
...
When your diagram contains custom WiFi access point parts, the default **Wokwi-GUEST** network is not created. For full details and attribute reference, see the [wokwi-wifi-ap part documentation](wokwi-wifi-ap.md "../parts/wokwi-wifi-ap.md").
...
| Availability | All users | [Paying users](https://wokwi.com/pricing?ref=docs_esp32_wifi "https://wokwi.com/pricing?ref=docs_esp32_wifi") |
...
If you are worried about running the gateway executable on your computer, you are invited to take a look at the [source code](https://github.com/wokwi/wokwigw "https://github.com/wokwi/wokwigw"), and even build the executable file yourself (ask for instructions on [discord](https://wokwi.com/discord "https://wokwi.com/discord")).
...
If your ESP32 project is an HTTP server, you can connect to it from your browser at [http://localhost:9080/](http://localhost:9080 "http://localhost:9080"). The connection will be forwarded by the gateway to the default HTTP port (80) on the simulated ESP32.
...
Note: The Private IoT Gateway is not currently supported in Safari due to [a technical limitation](https://bugs.webkit.org/show_bug.cgi?id=171934#c96 "https://bugs.webkit.org/show_bug.cgi?id=171934#c96"). Please use a different browser (e.g. Chrome, Firefox, Edge).
...
The MAC address of the simulated ESP32 is 24:0a:c4:00:01:10. You can specify a different MAC address using the [macAddress](esp32.html#changing-the-mac-address "esp32.md#changing-the-mac-address") attribute.
...
Wokwi simulates a complete network stack: starting at the lowest 802.11 MAC Layer, through the IP and TCP/UDP layers, all the way up to protocols such as DNS, HTTP, MQTT, CoAP, etc. You can view the raw WiFi traffic using a network protocol analyzer such as [Wireshark](https://www.wireshark.org "https://www.wireshark.org").
...
![ESP32 WiFi Wireshark Packets: DNS, HTTP, and 802.11 MAC](../assets/images/esp32-wifi-wireshark-5c4300f4facd21df5825daee51a03493.png)
...
* [NTP Client](https://wokwi.com/projects/321525495180034642 "https://wokwi.com/projects/321525495180034642") - Gets the current date and time from an NTP server and displays them on an LCD screen.
* [MicroPython MQTT Weather Logger](https://wokwi.com/projects/322577683855704658 "https://wokwi.com/projects/322577683855704658") - Reads the current temperature + humidity every second and report changes to an MQTT server.
* [ESP32 HTTP Server](https://wokwi.com/projects/320964045035274834 "https://wokwi.com/projects/320964045035274834") - Serves a web page that controls 2 LEDs. Requires the [Wokwi IoT Gateway](esp32-wifi.html#the-private-gateway "esp32-wifi.md#the-private-gateway").